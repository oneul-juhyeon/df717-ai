-- Clean up duplicate empty account_id entries and add unique constraint
DELETE FROM public.user_accounts WHERE account_id IS NULL OR account_id = '';

-- Now add unique constraint on account_id  
ALTER TABLE public.user_accounts ADD CONSTRAINT user_accounts_account_id_unique UNIQUE (account_id);

-- Backfill missing user accounts from Telegram data
