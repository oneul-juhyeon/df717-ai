...
const IsaacComponent = () => {
  ...
  useEffect(() => {
    if (inView1) controls1.start({ opacity: 1, y: 0, transition: { duration: 0.5, staggerChildren: 0.1 } });
  }, [inView1, controls1]);

  useEffect(() => {
    if (inView2) controls2.start({ opacity: 1, y: 0, transition: { duration: 0.5, staggerChildren: 0.1 } });
  }, [inView2, controls2]);

  useEffect(() => {
    if (inView3) controls3.start({ opacity: 1, y: 0, transition: { duration: 0.5, staggerChildren: 0.1 } });
  }, [inView3, controls3]);

  useEffect(() => {
    if (inView4) controls4.start({ opacity: 1, y: 0, transition: { duration: 0.5, staggerChildren: 0.1 } });
  }, [inView4, controls4]);
