...
const AxiTrustedPartner: React.FC = () => {
  ...
  return (
    <div className="flex max-lg:flex-col bg-gradient-to-br from-red-600 to-red-500 rounded-2xl overflow-hidden w-full">
      ...
      <div className="flex items-center justify-center p-6 bg-white/10 relative">
        <div className="absolute inset-0 bg-gradient-to-bl from-transparent to-red-600/40"></div>
        <div className="relative z-10 flex items-center justify-center">
          <div className="relative">
            {/* Doubled the size of the Shield icon from h-20 w-20 to h-40 w-40 */}
            <Shield className="text-white h-40 w-40 opacity-80 max-sm:h-32 max-sm:w-32" />
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              {/* Doubled the size of the logo from w-12 h-4 to w-24 h-8 */}
              <img src="/lovable-uploads/df717_logo.png" alt="DF717 Logo" className="w-24 h-8 max-sm:w-20 max-sm:h-6" />
            </div>
          </div>
        </div>
      </div>
